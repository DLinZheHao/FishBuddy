...
**FoundationModels (AI summary)**: `SpeciesDigestService` (singleton) + `SpeciesChatViewModel` (per-screen, message-timeline based) require iOS 26.0+ and Apple Intelligence enabled. App-level availability is exposed via `FoundationModelAvailabilityStore.shared`. Always guard with availability checks; the rest of the app must function without it.
